    ...
    internal class AndroidKey : AttestationFormat
    {
        ...
        public static byte[] GetASN1ObjectAtIndex(byte[] attExtBytes, int index)
        {
            ...
            if (null == attExtBytes || 0 == attExtBytes.Length || attExtBytes.Length > ushort.MaxValue)
                throw new VerificationException("Invalid attExtBytes signature value");
            var offset = 0;
            ...
            if (null == derSequence || 0x30 != derSequence[0])
                throw new VerificationException("attExtBytes signature not a valid DER sequence");

            // next is length of all the items in the sequence
            var dataLen = AuthDataHelper.GetSizedByteArray(attExtBytes, ref offset, 1);
            if (null == dataLen)
                throw new VerificationException("attExtBytes signature has invalid length");
            ...
            if (true == longForm)
            {
                ...
                if (null == longLenByte)
                    throw new VerificationException("attExtBytes signature has invalid long form length");
            ...
            for (var i = 0; i < index; i++)
            {
                ...
                if (null == derId)
                    throw new VerificationException("Ran out of bytes in attExtBytes sequence without finding the first octet string");
                var lenValue = AuthDataHelper.GetSizedByteArray(attExtBytes, ref offset, 1);
                if (null == lenValue)
                    throw new VerificationException("attExtBytes lenValue invalid");
                if (0 < lenValue[0])
                {
            ...
            if (null == lenAsn1value)
                throw new VerificationException("lenAttestationChallenge version length invalid");
        ...
        public static bool GetDERTagValue(byte[] authList, int tag, ref int result)
        {
            ...
            for (var i = 0; i < authList.Length;)
            {
                ...
                if (false == ((authList[i] & 0xA0) == 0xA0))
                    throw new VerificationException("Expected class 2 constructed ASN.1 value");
                ...
                {
                    throw new VerificationException("Expecting ASN.1 tag less than 0x3FFF");
                }
                ...
                if (tag == foundTag)
                {
                    ...
                    {
                        throw new VerificationException("Unexpected byte sequence found fetching ASN.1 integer value");
                    }
                }
        ...
        public override void Verify()
        {
            ...
            if (true != androidKeyPubKey.VerifyData(Data,
                ...
                throw new VerificationException("Invalid android key signature");
            ...
            if (false == clientDataHash.SequenceEqual(attestationChallenge))
                throw new VerificationException("Mismatched between attestationChallenge and hashedClientDataJson verifying android key attestation certificate extension");
