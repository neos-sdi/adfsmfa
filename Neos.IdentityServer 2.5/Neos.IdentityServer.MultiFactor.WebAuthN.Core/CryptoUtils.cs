...
using Neos.IdentityServer.MultiFactor.WebAuthN.Library.LCLib;
...
namespace Neos.IdentityServer.MultiFactor.WebAuthN
{
    public static class CryptoUtils
    {
        ...
        public static byte[] GetEcDsaSigValue(BinaryReader reader)
        {
            ...
            if (0x02 != derInt)
                throw new VerificationException("ECDsa signature coordinate sequence does not contain DER integer value"); // DER INTEGER
        ...
        public static byte[] SigFromEcDsaSig(byte[] ecDsaSig, int keySize)
        {
            using (var stream = new MemoryStream(ecDsaSig, false))
            {
                using (var reader = new BinaryReader(stream))
                {
                    ...
                    if (0x30 != derSequence) throw new VerificationException("ECDsa signature not a valid DER sequence");
                    ...
                    if (true == longForm)
                    {
                        ...
                        if (1 != longLenBytes)
                            throw new VerificationException("ECDsa signature has invalid long form data length bytes");
                        ...
                        if (0x80 > longLen)
                            throw new VerificationException("ECDsa signature has invalid long form data length");

                        // sanity check the length
                        if (ecDsaSig.Length != (reader.BaseStream.Position + longLen))
                            throw new VerificationException("ECDsa signature has invalid long form length");
                    }
                    ...
                    if (reader.BaseStream.Position != reader.BaseStream.Length)
                        throw new VerificationException("ECDsa signature has bytes leftover after parsing R and S values");
                    ...
                    if ((coefficientSize * 2) < (r.Length + s.Length))
                        throw new VerificationException("ECDsa signature has invalid length for given curve key size");
